...
Usage() {
	echo ""
	echo "Usage: roi_extract.sh <subjects_dir> <subjects_file> <rois_dir> <rois_file> <image_file> <smoothing>"
	echo ""
	echo "  <subjects_dir>  : Directory containing subject folders"
	echo "  <subjects_file> : Text file containing subject IDs"
	echo "  <rois_dir>      : Directory containing ROI files"
	echo "  <rois_file>     : Text file containing ROI names"
	echo "  <image_file>    : File name and extension of MRI to use as input"
	echo "  <smoothing>     : Smoothing kernel of input image"
	echo ""
	exit 1
}
...
for roi in `more ${rois_file}` ; do
	...
	echo "f.close()" >> runner
	echo "line = subject_id + ',' + line.strip().replace('  ', ',')" >> runner
